...
void page::set_cache(page* pagep, kmem_cache_t* cachep) {
    // 设置页面对象所属的缓存对象
    if (pagep == nullptr || cachep == nullptr) {
        error_t err;
        set_error(&err, NULL_POINTER, "Page or Cache pointer is null", __func__);
        print_error(&err);
        return;
    }
...
kmem_cache_t* page::get_cache(page* pagep) {
    ...
    if (pagep == nullptr) {
        error_t err;
        set_error(&err, NULL_POINTER, "Page pointer is null", __func__);
        print_error(&err);
        return nullptr;
    }
...
void page::set_slab(page* pagep, slab* slabp) {
    // 设置页面对象所属的slab对象
    if (pagep == nullptr || slabp == nullptr) {
        error_t err;
        set_error(&err, NULL_POINTER, "Page or Slab pointer is null", __func__);
        print_error(&err);
        return;
    }
...
page* page::virtual_to_page(void* vir) {
    ...
    if (index >= bud->use) {
        error_t err;
        set_error(&err, BUDDY_SYSTEM_OVERFLOW, "Index out of buddy system range", __func__);
        print_error(&err);
        return nullptr; // 如果索引超出伙伴系统可用范围，则返回空指针
    }
