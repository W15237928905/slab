...
static void init_cache() {
	...
	new(&cache_cache.spinlock) recursive_mutex(); // 初始化互斥锁
}
...
int kmem_cache_shrink(kmem_cache_t* cachep) {
	...
	lock_guard<recursive_mutex> guard(cachep->spinlock); // 锁定缓存的互斥锁
	...
	while (cachep->slabs_free.next != nullptr) {
		...
		bud->kmem_freepages(adr, cachep->gfporder); // 释放内存
		cnt += cachep->gfporder; // 增加释放的块数
