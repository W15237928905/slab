...
buddy::buddy(void* space, unsigned long long size) {
    new(&this->spinlocked) recursive_mutex(); // 初始化递归互斥锁
    int need_space = 1;
    while ((sizeof(buddy) + sizeof(page) * (size - need_space) + sizeof(list_head) * ((int)log2(size - need_space) + 1)) > BLOCK_SIZE * need_space) {
        need_space++;
    }
    size -= need_space;
    this->use = (int)size;
    this->space = (void*)((unsigned long long)space + need_space * BLOCK_SIZE);
...
int buddy::kmem_freepages(void* from, unsigned long long order) {
    ...
    lock_guard<recursive_mutex> guard(spinlocked); // 锁定互斥锁
    list_head* tmp;
