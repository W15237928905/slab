...
>  本项目实现了一个基于Linux内核Slab分配器的面向对象内存分配系统。
...
> 分配内存
>
![image](img/分配内存代码.png)
![image](img/分配内存.png)
>
>释放内存
>
![image](img/释放内存代码.png)
![image](img/释放内存.png)
>
>销毁内存
>
![image](img/销毁内存代码.png)
![image](img/销毁内存.png)
>
### 3.3.3 error模块
>
...
>
![image](img/main函数.png)
![image](img/main函数代码.png)
>
## 3.4 安全
>
...
>
# 5.创新点
>(1) 使用check函数，用于验证分配的内存数据是否符合预期，确保数据的一致性
>
>(2) 使用run_threads函数模拟多个线程同时进行内存分配和释放操作，测试系统在并发情况下的性能和稳定性
>
>(3) 在多线程环境下，通过合理的锁机制和内存管理策略，优化内存分配和释放的效率，确保系统在高并发情况下仍能高效运行。
>
>(4) 对异常和错误使用结构体 `error` 来封装处理
>
...
## 6.1 问题及解决
>
>  在本次项目设计初始阶段使用的是最基础的dev-c++进行编译。而使用此编译器进行编译时会出现无法成功编译的报错信息，提示出现：#error This file requires 
>  compiler and library support for the ISO C++ 2011。
...
## 6.2 存在的不足 
>
>  此程序仍存在一些不足之处，如样本数较少，无法判断是否适用于不同程序开发环境，错误种类较少，无法显示输出所有程序运行错误信息。程序不能实时更新错误种类，若出现不在error类中枚举的错误类型，程序显示错误，但无法定位错误位置。缺乏错误恢复机制（如自动重试、回退操作等）。在复杂系统中，一个错误可能引发其他错误等。
>
>
## 6.3 收获和展望 
>
>  经过此次项目的研发测试，我们对操作系统中的伙伴系统以及slab分配算法有了更深刻的理解，对操作系统如何分配内存有了更加清晰地认识。我们了解了伙伴系统是如何管理物理内存分配。同时，我们也理解slab分配算法的原理及其实现。
>
>在未来的工作和学习中，我们计划进一步优化内存管理算法，增强系统的容错能力，并探索更多的内存管理策略，如内存池和垃圾回收等，提升系统的整体性能和稳定性。考虑引入错误链（chained errors）的概念，将多个相关的错误串联在一起，便于追踪错误的根源。
